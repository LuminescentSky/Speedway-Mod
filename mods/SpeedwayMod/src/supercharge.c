...
bool IsInAnyFlyingState(void) {
    return (_spyro.state == SUPERFLY || _spyro.state == SUPERFLY_ASCENDING || _spyro.state == SUPERFLY_DESCENDING);
}
...
void SuperchargeUpdate(void)
{
    ...
    if (_gameState == GAMESTATE_GAMEPLAY && !_spyro.isMovementLocked && modOptions[1]) {
    ...
    if (modOptions[1] || _isInFlightLevel) {
        _spyro.glideUsed = 0;
    }
    ...
    if (_gameState == GAMESTATE_GAMEPLAY) {
        ...
        /*if (_currentButtonOneFrame & CIRCLE_BUTTON) {
            _gemsPerLevel[realLevelIDIndex] = 2000;
            _dragonsPerLevel[realLevelIDIndex] = 5;
        }*/
        ...
        if (_spyro.state == CHARGING_AIR) {
            if (_currentAnimationState == 3 && !isSupercharging) {
                ...
                if (chargeTransitionFlag > 0) {
                    ...
                    chargeTransitionMomentum = (-(chargeTransitionTime*chargeTransitionTime)) * 16 + chargeTransitionMomentumStart;
            ...
            if (_currentAnimationState != 3 && (_currentButton & SQUARE_BUTTON)) {
...
void SharpTurning(void) {
    if (_gameState == GAMESTATE_GAMEPLAY) {
        ...
        if ((!((~_rawButtons & RAW_RIGHT) && (~_rawButtons & RAW_DOWN) || (~_rawButtons & RAW_LEFT) && (~_rawButtons & RAW_DOWN)) && _rawLeftAnalogY < 0xc0) || _spyro.state != SUPERCHARGE) {
            sharpTurnFlag = 0;
        }
