...
void BeforeMenuText() {
    if (_gameState == GAMESTATE_PAUSED) {
        if (!selectedQuitGame) {
            if (_altLevelID % 10 == 5) {
                memcpy(&_ptr_quit, &_ptr_exitLevel, 36);
            } else {
                memcpy(&_ptr_exitLevel, &_ptr_quit, 36);
            }
        } else {
            if (_altLevelID % 10 == 5) {
                memcpy(&_ptr_quit, &_ptr_quitGame, 36);
            } else {
                memcpy(&_ptr_exitLevel, &_ptr_quitGame, 36);
            }
        }
    }
    if (_gameState == GAMESTATE_INVENTORY) {
        if (guidebookDialogState == INVENTORY) {
            ...
            //_midInventoryHookAddress = 0xd6c2000c;
            _midInventoryHookAddress = 0x0c00c2d6;
        } else {
            //_midInventoryHookAddress = 0x9a830008;
    ...
    if (_gameState == GAMESTATE_INVENTORY) {
        if (guidebookDialogState == INVENTORY) {
            if (CheckRegion() == NTSC) {
                ...
                DrawIGT(0x10, 8, 0, false, MOBY_COLOR_GOLD, LEFT);
            }
            ...
            if (IGTpage == 1) {
                if (endingType == 1) {
                    ...
                    // 120% GAME COMPLETE
                    DrawTextAll(
                        stringList[completionString],
                        &(CapitalTextInfo){.x = completionPosition, .y = 0x58, .size = DEFAULT_SIZE},
                        &(LowercaseTextInfo){.spacing = DEFAULT_SPACING, .yOffset = 0, .size = DEFAULT_SIZE},
                        DEFAULT_SPACING,
                        MOBY_COLOR_PURPLE
                    );
